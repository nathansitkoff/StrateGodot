...
func start_setup(team: PieceData.Team) -> void:
	current_team = team
	selected_rank = -1
	placed_counts.clear()
...
func _build_tray() -> void:
	# Clear existing tray buttons
	for child: Node in piece_tray.get_children():
		child.queue_free()

	# Create a button for each rank
	for rank: int in PieceData.RANK_INFO:
...
func _refresh_tray() -> void:
	...
	# Enable ready button when all pieces placed
...
func _on_square_clicked(pos: Vector2i) -> void:
	...
	# If clicking an occupied cell, remove the piece
	if existing != -1:
	...
	# Auto-deselect if no more of this rank
	if placed_counts[selected_rank] >= PieceData.RANK_INFO[selected_rank]["count"]:
		selected_rank = -1
...
func _on_randomize_pressed() -> void:
	...
	# Build list of remaining pieces to place
	...
	# Place remaining pieces in shuffled empty cells
